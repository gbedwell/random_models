...
```{r}
frag.directory <- paste0(getwd(), "/nasb_fragmentation/")
...
#### Calculate outliers
```{r}
outliers <- rigrag::pattern_frag_model(df = resampled.unique.genes, pattern="NASB") %>%
...
```{r, eval=FALSE}
model_plot(rigrag::pattern_frag_model(df = resampled.unique.genes, pattern="NASB") %>%
             dplyr::filter(total.sites == 700000, iteration == 6) %>%
...
```{r, eval=FALSE}
systematic.lower.outliers <- rigrag::pattern_frag_model(df = resampled.unique.genes, pattern="NASB") %>%
...
write.table(systematic.lower.outliers, 
            "~/Documents/scripts/bioinformatics/int_sites/int_site_analysis/models/model_files/nasb_systematic_rags.txt",
            sep="\t", quote=FALSE, row.names = FALSE, col.names = FALSE)
```

```{r}
save.image(file = paste0("~/Documents/scripts/bioinformatics/int_sites/int_site_analysis/models/model_files/",
                         "nasb_model_assessment.RData"), compress=TRUE, safe = F)
```
...
nasb.outliers <- outliers

rm(outliers)

save.image(file = paste0("~/Documents/scripts/bioinformatics/int_sites/int_site_analysis/models/model_files/",
                         "/nasb_outlier_plot_df.RData"), compress=TRUE, safe = F)
```
