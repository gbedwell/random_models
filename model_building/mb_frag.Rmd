...
```{r, eval=FALSE}
rm(list=setdiff(ls(), c("collapsed.genes", "scratch.uniq.df", "unique.genes", "summary.genic.frac", 
...
int.fit.temp <- summary(nlsLM(mean.intercept ~ (N*total.sites^a) + k,
      ...
      start = list(N = max(mean.linfits$mean.intercept),
                   a = unname(coef(lm(log(mean.intercept) ~ log(total.sites), data=mean.linfits))[2]),
                   k = 0),
      control = list(maxiter = 500)))
...
#### Estimate the spread for each random integration site dataset
...
+ Parameters hard to estimate based on other metrics
  + Function throws singularity errors
  + Based initial parameters on previous fits of the same data to achieve optimization
...
#### Save the RData file
```{r}
save.image(file = paste0(frag.directory,"/mb_fragmentation.RData"), safe = F)
```
...
save.image(file = paste0("~/Documents/scripts/bioinformatics/int_sites/int_site_analysis/models/model_files",
                         "/plots_mb_fragmentation.RData"), safe = F)
```
